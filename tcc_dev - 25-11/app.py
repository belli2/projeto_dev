...
def index1():
    ...
    # Adicione a condição para filtrar status
    ...
    # Aplique a formatação da data e hora em cada ordem
    ordens_formatadas = [
    ...
    #     prioridade = ordem[4]
    #     categoria = "Não definida" if prioridade == 0 else "Alta" if prioridade == 1 else "Média" if prioridade == 2 else "Baixa"
    #     lista_ordenada = sorted(todas_ordens)
    #     # ordem_atual = list(ordem)
    #     lista_ordenada.append(categoria)
    #     todas_ordens.append(lista_ordenada)
...
#     # Exemplo de uma função que você pode usar para formatar a data
#     def format_data_to_ptbr(data):
#         if data:  # Verifique se a data não é None
#             return datetime.strptime(data, '%Y-%m-%d').strftime('%d/%m/%Y')
#         return data  # Retorna None ou vazio se a data for inválida

#     # Exemplo de como você pode aplicar isso nas ordens antes de passar para o template
#     ordens_formatadas = [(format_data_to_ptbr(ordem[1]), ordem[2], ordem[3]) for ordem in ordens]
